...
class NewsScreenOld():
    ...
    def __init__(self, windowres, fade_direction=""):
        ...
        self.show_fetching = False



    def Update(self, InputEvents):
        self.show_fetching = False
        if "RUNNING" in self.ScreenStatus:
            ...
            if self.ScreenStatus is "RUNNING":
                ...
                if not self.lemonde_fetched:
                    ...
                    self.animation.reset()
        elif "FADING_IN" in self.ScreenStatus:
            self.fade_in()
    ...
    def fade_in(self):
        time = self.animation.elapsed_time()

        if time > 1:
            self.ScreenStatus = "RUNNING"
            self.show_fetching = True
