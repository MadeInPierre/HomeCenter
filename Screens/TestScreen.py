...
class TestScreen():
    ...
    def __init__(self, windowres):
        ...
        # Charger une image
        self.sunny_icon=pygame.image.load(
            "Images/weather_sunny_transparent.png").convert_alpha()
        ...
        self.TitleFont2=pygame.font.Font(
            "Fonts/HelveticaNeue-UltraLight.ttf", 100)
        self.WaitFont=pygame.font.Font(
            "Fonts/HelveticaNeue-UltraLight.ttf", 22)
        self.PostTitleFont=pygame.font.Font(
            "Fonts/HelveticaNeue-Medium.ttf", 12, bold=False)

        self.Aujourdhui_color=0

        self.TemperatureText=self.TitleFont2.render(
            "15 C", True, (255, 255, 255))

        self.chrono=AnimationManager()

    def Update(self, InputEvents):
        if self.chrono.elapsed_time() > 0 and self.chrono.elapsed_time() < 1:
            self.Aujourdhui_color=int(
                255 / (1 + math.exp(-(self.chrono.elapsed_time() - 0.5) / 0.1)))


        if self.chrono.elapsed_time() > 1 and self.chrono.elapsed_time() < 3:
            self.sunny_positionX=int(
                80 + 500 / (1 + math.exp(-(self.chrono.elapsed_time() - 2) / 0.2)))
        if self.chrono.elapsed_time() > 1 and self.chrono.elapsed_time() < 3:
            self.sunny_positionY=int(
                100 + 100 / (1 + math.exp(-(self.chrono.elapsed_time() - 2) / 0.2)))
    ...
    def Draw(self, gameDisplay):
        ...
        gameDisplay.blit(
            self.sunny_icon, (self.sunny_positionX, self.sunny_positionY))

        self.AujourdhuiText=self.TitleFont.render(str(self.chrono.elapsed_time(
            )), True, (self.Aujourdhui_color, self.Aujourdhui_color, self.Aujourdhui_color))
        gameDisplay.blit(self.AujourdhuiText, (55, 30)
                         )  # pos initiale : 55, 30
